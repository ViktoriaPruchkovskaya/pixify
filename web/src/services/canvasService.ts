...
export default class CanvasService {
    ...
    public async addCanvas(canvas: Canvas, name: string): Promise<void> {
        ...
        const transaction = storeService.db.transaction(this.storeName, "readwrite");
        const existingCanvas = await storeService.get({storeName: this.storeName, key: name, transaction});
        if (existingCanvas) {
    ...
    public async getCanvasByName(name: string): Promise<CanvasEntry> {
        const storeService = await StorageService.getInstance();
        return storeService.get<CanvasEntry>({storeName: this.storeName, key: name});
    }
}